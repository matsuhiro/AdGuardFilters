// ==UserScript==
// @name         AdGuard Parental Control Checker
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  ファイル内ドメインを一括チェックし、ブロックされないドメインを抽出
// @match        https://adguard-dns.io/*/dashboard/settings/*/userrules
// @grant        none
// ==/UserScript==
      ...
      for (let raw of lines) {
        ...
        await new Promise(res => {
          const obs = new MutationObserver((_, o)=>{
            if (document.querySelector('.Nric9fWIgpVPG57wp_cn')) {
              obs.disconnect();
              res();
            }
          });
        ...
        // 「ペアレンタルコントロール」が含まれなければ未ブロック
        if (!resp.includes('ペアレンタルコントロール')) {
          notBlocked.push(raw);
        }
        // サーバ負荷軽減のため少し待機
